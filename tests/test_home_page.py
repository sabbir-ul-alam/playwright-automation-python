...
def test_verify_scrollup_functionality_using_arrow_icon(page: Page):
    homepage = HomePage(page)
    homepage.visit()
    assert homepage.is_loaded()

    locator = page.locator('h2').get_by_text('Subscription')
    locator.scroll_into_view_if_needed()
    assert locator.is_visible()

    page.locator('#scrollUp').click()
    text = page.get_by_text('Full-Fledged practice website for Automation Engineers').first
